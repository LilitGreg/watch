...
class DOD_Custom_Fields {
	...
	public static function init() {
		...
		add_action('wp_enqueue_scripts', array(__CLASS__, 'admin_scripts'));
	...
	public static function admin_scripts(){
		if (is_admin() && strpos($_SERVER['PHP_SELF'], 'post.php')) {
			...
			wp_enqueue_style('validate-css', '/assets/css/cmxform.css', array(), '', false );
			wp_enqueue_script('jquery-validate-min', '/assets/js/admin/jquery.validate.min.js', array(), '', true);
			wp_enqueue_script('post-validate', '/assets/js/admin/post_validate.js', array(), '', true);
			
		}
	}
	/**
	 * saving the custom fields data the product.
	 */
	...
	/**
	 * script callback to add custom js to product create/edit page
	 */
	...
	/**
	 * adding custom box to show custon fields in product create/edit page
	 */
	...
	public static function show_custom_meta_box() {
		...
		foreach (self::$custom_meta_fields as $field) {
			...
			switch ($field['type']) {
				case 'text':
				if($field['id'] == 'dod_deal_price'){
						esc_html_e('Product sales price will be the deal price.','tlsmw');
					}
					else{
